...
if __name__ == '__main__':
    ...
    for i in range(output_units):
        connect_map_i, connect_area_num_i = skimage.measure.label(np.int32(conf_img_label == i + 1), connectivity=2,
                                                              background=0, return_num=True)
        connect_map_lst.append(connect_map_i)

    for i in range(len(connect_map_lst)-1):
        t1 = connect_map_lst[i+1] > 0 + 0
        connect_map_lst[i+1] = t1 * connect_map_lst[i].max() + connect_map_lst[i+1]
    connect_area_num = connect_map_lst[-1].max()
    connect_map = np.sum(np.asarray(connect_map_lst), 0)
    ...
    for i in range(connect_area_num):
        ...
        area_train_label = []
        sum_score = []
        for j in range(len(connect_area_index)):
             selected_connect_map[connect_area_index[j]] = conf_img_label[connect_area_index[j]]
        # if com_index != []:
        #     for j in range(len(connect_area_index)):
        #         selected_connect_map[connect_area_index[j]] = conf_img_label[connect_area_index[j]]
        # else:
