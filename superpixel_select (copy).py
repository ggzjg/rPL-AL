...
if __name__ == '__main__':
    ...
    train_index = np.load('./Split_Data/{}_{}_shot_train_index.npy'.format(args.dataset, args.perclass)).tolist()
    for i in range(len(train_index)): train_index[i] = tuple(train_index[i])  # to tuple
    conf_index = np.load('./Split_Data/pseudo_thres_index_fs'+str(args.perclass)+'_iter'+args.iter_idx+'.npy')
    conf_index2label = np.load('./Split_Data/pseudo_ind2label_fs'+str(args.perclass)+'_iter'+args.iter_idx+'.npy').tolist()
    pseudo_ind2score = np.load('./Split_Data/pseudo_ind2score_fs'+str(args.perclass)+'_iter'+args.iter_idx+'.npy').tolist()
    ...
    connect_map, connect_area_num = skimage.measure.label(conf_img_label, connectivity=2, background=0, return_num=True)
    ...
    for i in range(connect_area_num):
        ...
        if com_index != []:
            ...
            if len(list(set(area_train_label))) > 1:
                ...
                connect_map[connect_map == i + 1] = select_label
            else:
                connect_map[connect_map == i + 1] = area_train_label[0]
        else :
            # delete connect area
            connect_map[connect_map == i + 1] = 0

    # get final index
    final_connect_index = list(zip(np.nonzero(connect_map)[0], np.nonzero(connect_map)[1]))
    np.save('./Split_Data/final_connect_pseudolabel_fs'+str(args.perclass)+'_iter'+args.iter_idx+'.npy', connect_map)
    ...
    sio.savemat('./Split_Data/final_connect_pseudolabel_fs'+str(args.perclass)+
                '_iter'+args.iter_idx+'.mat', {'connect_map':connect_map})
