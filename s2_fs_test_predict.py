...
if __name__ == '__main__':
    ...
    # gt_label
    test_label = np.load('./Split_Data/{}_{}_shot_test_label.npy'.format(args.dataset, args.perclass))
    wrong_label = pred_label - test_label
    wrong_num = 0
    for i in range(len(wrong_label)):
        if 0 != wrong_label[i]:
            wrong_num +=1
    print("wrong label: ", wrong_num)
    print("test label", len(wrong_label))
    print('\n')
    if args.iter_idx == '1':
